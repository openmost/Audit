...
class UniqueVisitorsRangeCheck extends AbstractCheck
{
    public function execute(ChecklistItem $item): CheckResult
    {
        ...
        $value = (string) ($general['enable_processing_unique_visitors_range'] ?? '0');

        return $this->pass($item,
            detail: $this->t($item, 'pass', ['current' => $value],
                "enable_processing_unique_visitors_range = {$value}."),
            currentValue: $value,
